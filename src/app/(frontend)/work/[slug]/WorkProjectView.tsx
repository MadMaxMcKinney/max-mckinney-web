...
import type { WorkProject } from "@payload-types";
...
/**
 * Renders a work project and, when shown inside the Payload admin Live Preview
 * iframe, updates in real time as the editor types — `useLivePreview` merges the
 * draft form state (and repopulates relationships via the REST API) over
 * `initialData` without waiting for a save or a route refresh. For normal
 * visitors no preview messages arrive, so it just renders `initialData`.
 */
export default function WorkProjectView({ initialData, serverURL }: { initialData: WorkProject; serverURL: string }) {
    const { data: fm } = useLivePreview<WorkProject>({ initialData, serverURL, depth: 2 });
