 ...
 */
export default function Tooltip({ content, children, as: Component = "span", className }: TooltipProps) {
    const { show, hide } = useTooltipController();

    const onPointerEnter = useCallback(
        (event: React.PointerEvent) => show(content, event.clientX, event.clientY),
        [show, content]
    );
